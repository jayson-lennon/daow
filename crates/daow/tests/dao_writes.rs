...
use daow::{async_trait, dao, ExecuteResult, FromSqlColumn, Pool, Result, ToSqlColumn};
...
impl FromSqlColumn for Slug {
    fn from_column(value: &daow::ColumnValue) -> daow::Result<Self> {
...
impl ToSqlColumn for Slug {
    fn to_column(&self) -> daow::Result<daow::Param> {
        self.0.to_column()
    }
}
...
async fn dao_execute_destructured_with_scalar() {
    ...
    let pool = daow::Pool::open(db_path.to_str().unwrap()).unwrap();
    let dao = DestructuredDao::new(pool.clone());
...
async fn dao_query_destructured() {
    ...
    pool.execute("INSERT INTO items (id, name, price) VALUES (?, ?, ?)", daow::ToRow::to_insert_params(&item1).unwrap()).await.unwrap();
    pool.execute("INSERT INTO items (id, name, price) VALUES (?, ?, ?)", daow::ToRow::to_insert_params(&item2).unwrap()).await.unwrap();
    pool.execute("INSERT INTO items (id, name, price) VALUES (?, ?, ?)", daow::ToRow::to_insert_params(&item3).unwrap()).await.unwrap();
...
async fn dao_execute_destructured_complete() {
    ...
    pool.execute(
        ...
        daow::ToRow::to_insert_params(&Item { id: 1, name: "old".to_string(), price: 5.00 }).unwrap(),
    ).await.unwrap();
