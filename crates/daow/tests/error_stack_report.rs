...
//! `Result<T, Report<daow::Error>>`), the macro-emitted bodies correctly convert
//! `daow::Error` into the declared error type, preserving the original error in
//! the `Report` frame chain.

use daow::{async_trait, dao, ExecuteResult, Pool};
...
// --- Entity ---
#[derive(Debug, Clone, PartialEq, daow::Entity)]
#[dao(table = "widgets")]
struct Widget {
    #[dao(pk)]
    id: i64,
...
trait WidgetDao {
    ...
    // `name`. Forces a daow::Error through the Option<T> query tail.
...
// ===========================================================================
// DAO whose methods return Report<daow::Error> (the special-case slot).
// ===========================================================================
#[dao]
#[async_trait]
trait WidgetDaoDaoErr {
    #[insert]
    async fn insert(&self, widget: Widget) -> Result<ExecuteResult, Report<daow::Error>>;

    #[execute("INSERT INTO widgets (id, name) VALUES (?, ?)")]
    async fn insert_dup(&self, widget: Widget) -> Result<ExecuteResult, Report<daow::Error>>;
}

// ===========================================================================
// DAO whose methods return plain daow::Result (regression — today's behavior).
// ===========================================================================
#[dao]
#[async_trait]
trait WidgetDaoPlain {
    #[insert]
    async fn insert(&self, widget: Widget) -> daow::Result<ExecuteResult>;

    #[query("SELECT id, name FROM widgets WHERE id = ?")]
    async fn get_by_id(&self, id: i64) -> daow::Result<Option<Widget>>;
}
...
// --- helper: assert a Report<StoreError> carries a daow::Error in its frame chain ---
fn assert_chain_reaches_dao(report: &Report<StoreError>) {
    ...
    let reaches_dao = report
        .frames()
        .any(|f| f.downcast_ref::<daow::Error>().is_some());
    assert!(
        reaches_dao,
        "frame chain should contain the original daow::Error"
    );
}
...
async fn report_unit_error_option_query_failure() {
    // The Option<T> query path (ReturnKind::Option) must also wrap daow::Error into
...
// ===========================================================================
// Report<daow::Error> slot — special-case path (.map_err(Report::new)).
// ===========================================================================
#[tokio::test]
async fn report_dao_error_execute_failure() {
    ...
    let err = dao
        ...
        .unwrap_err();
    assert!(
        err.downcast_ref::<daow::Error>().is_some(),
        "Report<daow::Error> top context should be daow::Error"
    );
}

// ===========================================================================
// Plain daow::Result regression — today's behavior unchanged.
// ===========================================================================
#[tokio::test]
