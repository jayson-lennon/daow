...
//! instead of the `daow::Result` alias.
//!
...
/// underlying `daow::Error` into this type via `change_context`.
#[derive(Debug)]
struct UserError;
...
async fn setup_db() -> Result<Pool, daow::Error> {
...
impl FromSqlColumn for UserId {
    fn from_column(value: &ColumnValue) -> Result<Self, daow::Error> {
        Ok(UserId(i64::from_column(value)?))
    }
}

impl ToSqlColumn for UserId {
    fn to_column(&self) -> Result<daow::Param, daow::Error> {
        self.0.to_column()
    }
}
...
async fn main() -> Result<(), Report<UserError>> {
    // `setup_db` returns `daow::Result`, so bridge it into our domain error
    // with `change_context`.
