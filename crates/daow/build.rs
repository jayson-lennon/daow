...
fn main() {
    ...
    // `cargo clean`) and expose its path as DAOW_DATABASE_URL so the #[dao] macro
    ...
    // without needing their own DAOW_DATABASE_URL wiring.
    println!("cargo:rustc-env=DAOW_DATABASE_URL={}", db_path.display());
}
