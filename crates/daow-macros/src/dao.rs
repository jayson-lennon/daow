...
struct DaoMethod {
    ...
    /// How the method's error slot is declared (plain `daow::Result`, `Report<C>`, or
    /// `Report<daow::Error>`). Drives error_stack conversion code emission.
...
enum ErrorSlot {
    /// `Result<T>` or `Result<T, daow::Error>` — today's behavior, no error_stack code.
    Plain,
    /// `Result<T, Report<C>>` where `C` is a (unit) error type the consumer provides.
    /// The generated body emits `.change_context(C)`.
    Report(Type),
    /// `Result<T, Report<daow::Error>>` — special case: `daow::Error` is an enum, not a
...
pub fn dao_impl(_attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let expanded = quote! {
        ...
        impl #struct_name {
            /// Construct from anything that converts into a [`daow::Conn`] — typically a
            /// [`daow::Pool`] (autocommit) but a [`daow::Transaction`] is also accepted.
            pub fn new<C: ::std::convert::Into<daow::Conn>>(conn: C) -> Self {
                Self { conn: conn.into() }
            }
            ...
            ///       live (E0505).
            pub fn with<'a>(&'a self, tx: &'a daow::Transaction) -> #view_name<'a> {
                #view_name {
                    conn: tx,
            ...
            pub async fn query_one<T: daow::FromRow + Send + 'static>(&self, sql: &str, params: Vec<daow::Param>) -> daow::Result<Option<T>> {
                self.conn.query_one(sql, params).await
            }

            /// Pass-through helper: run a query returning many rows.
            pub async fn query_all<T: daow::FromRow + Send + 'static>(&self, sql: &str, params: Vec<daow::Param>) -> daow::Result<Vec<T>> {
                self.conn.query_all(sql, params).await
            }

            /// Pass-through helper: run an execute (INSERT/UPDATE/DELETE/DDL).
            pub async fn execute(&self, sql: &str, params: Vec<daow::Param>) -> daow::Result<daow::ExecuteResult> {
                self.conn.execute(sql, params).await
            }
        }

        /// Transaction-scoped view of [`#struct_name`]. Borrows the [`daow::Transaction`] for
...
///
/// Returns:
/// - `Plain` for `Result<T>`, `Result<T, daow::Error>`, or non-`Result` returns.
/// - `Report(C)` for `Result<T, Report<C>>` where `C` is not `daow::Error`.
/// - `ReportDaoError` for `Result<T, Report<daow::Error>>` (special case: `daow::Error`
...
fn analyze_error_slot(full_return_type: &Type) -> ErrorSlot {
    ...
    // Result<T> (single arg) defaults the error to daow::Error — plain.
...
/// True if `ty`'s path resolves to `daow::Error`.
fn path_is_dao_error(ty: &Type) -> bool {
    ...
    let segments: Vec<_> = type_path
        ...
        .collect();
    // Accept `daow::Error`, `crate::daow::Error`, etc. — match the last two segments.
    segments.len() >= 2 && segments[segments.len() - 2] == "daow" && segments[segments.len() - 1] == "Error"
}
...
///
/// For `Report(C)`: emit `expr.change_context(C)` — `ResultExt` lifts the `daow::Error` into a
/// `Report<daow::Error>` internally, then attaches `C` as the new top context. The trailing `?`
...
/// For `ReportDaoError`: emit `expr.map_err(::error_stack::Report::new)` — `daow::Error` is an
/// enum, not a unit value, so `change_context(daow::Error)` is impossible.
...
fn validate_sql(methods: &mut [DaoMethod]) -> syn::Result<()> {
    ...
    let db_url = match std::env::var("DAOW_DATABASE_URL") {
        Ok(url) => url,
        Err(_) => {
            return Err(syn::Error::new(
                proc_macro2::Span::call_site(),
                "DAOW_DATABASE_URL environment variable not set. \
...
fn generate_query_method(method: &DaoMethod) -> proc_macro2::TokenStream {
    ...
    let tail = match method.return_kind {
        ...
        ReturnKind::Bare => quote! {
            self.conn.query_one(#sql, #param_bindings).await
                .and_then(|opt| opt.ok_or_else(|| daow::Error::custom("query returned no rows")))
        },
    };
...
fn generate_insert_method(method: &DaoMethod) -> proc_macro2::TokenStream {
    ...
    let params_expr = wrap_fallible(
        &method.error_slot,
        quote! { daow::ToRow::to_insert_params(&#param_name) },
    );
    let tail = quote! {
        self.conn.execute(
            <#entity_type as daow::EntityMeta>::insert_sql(),
            #params_expr?,
        ).await
    };
    let wrapped_tail = wrap_tail(&method.error_slot, tail);
...
fn generate_upsert_method(method: &DaoMethod) -> proc_macro2::TokenStream {
    ...
    let tail = quote! {
        self.conn.execute(
            <#entity_type as daow::EntityMeta>::upsert_sql(),
            #params_expr?,
        ).await
    };
    let wrapped_tail = wrap_tail(&method.error_slot, tail);
...
fn generate_update_method(method: &DaoMethod) -> proc_macro2::TokenStream {
    ...
    let params_expr = wrap_fallible(
        &method.error_slot,
        quote! { daow::ToRow::to_update_params(&#param_name) },
    );
    let tail = quote! {
        self.conn.execute(
            <#entity_type as daow::EntityMeta>::update_sql(),
            #params_expr?,
        ).await
    };
    let wrapped_tail = wrap_tail(&method.error_slot, tail);
...
fn generate_delete_method(method: &DaoMethod) -> proc_macro2::TokenStream {
    ...
    let params_expr = wrap_fallible(
        &method.error_slot,
        quote! { daow::ToRow::to_delete_params(&#param_name) },
    );
    let tail = quote! {
        self.conn.execute(
            <#entity_type as daow::EntityMeta>::delete_sql(),
            #params_expr?,
        ).await
    };
    let wrapped_tail = wrap_tail(&method.error_slot, tail);
...
fn generate_execute_method(method: &DaoMethod) -> proc_macro2::TokenStream {
    ...
    let body = if let Some(sql_param_count) = method.sql_param_count {
        ...
        let assert_stmt = quote! {
            const _: () = assert!(
                <#entity_type as daow::EntityMeta>::FIELD_COUNT == #count_literal,
...
///
/// Each `to_column()` call is wrapped via `wrap_fallible` so that its `daow::Error` is converted
...
fn generate_param_bindings(method: &DaoMethod) -> proc_macro2::TokenStream {
    let to_col = |binding: &syn::Ident| {
        let call = quote! { daow::ToSqlColumn::to_column(&#binding) };
