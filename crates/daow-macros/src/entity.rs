...
pub fn derive_entity_impl(input: TokenStream) -> TokenStream {
    ...
    let from_row_impl = quote! {
        impl #impl_generics daow::FromRow for #struct_name #ty_generics #where_clause {
...
fn generate_write_impls(
    ...
    let insert_param_exprs = field_infos.iter().map(|info| {
        let ident = &info.field_ident;
        quote! { daow::ToSqlColumn::to_column(&self.#ident)? }
    });
    ...
    let to_row_impl = quote! {
        impl #impl_generics daow::ToRow for #struct_name #ty_generics #where_clause {
            fn to_insert_params(&self) -> daow::Result<Vec<daow::Param>> {
                Ok(vec![#(#insert_param_exprs),*])
            }

            fn to_update_params(&self) -> daow::Result<Vec<daow::Param>> {
                Ok(vec![#(#update_param_exprs),*])
            }

            fn to_delete_params(&self) -> daow::Result<Vec<daow::Param>> {
    ...
    let entity_meta_impl = quote! {
        impl #impl_generics daow::EntityMeta for #struct_name #ty_generics #where_clause {
